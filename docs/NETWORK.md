...
**State:** full layout, HQ VLAN assignments, LACP trunks and VLAN 30 STP roles reviewed. Exact-checkpoint save/reopen and routing/service validation remain pending. This is the source for topology, addressing, and traffic intent; recorded observations are in [Validation](VALIDATION.md).
...
Target: **20 devices and 21 physical links**. Logical port-channels do not add physical links. Supplied HQ-L3 output identifies `WS-C3560-24PS`, simulated IOS `12.2(37)SE1`, and physical ports F0/1–24 and G0/1–2. The five used local ports and corresponding CDP remote port numbers match the table below; repeated default neighbor names do not uniquely identify HQ-SW1 versus HQ-SW2. Both LACP bundles have operational evidence; member-failure tests and runtime checks for IP routing, SVIs, OSPF, relay and ACLs remain pending. [Packet Tracer switch guide](https://tutorials.ptnetacad.net/help/default/config_switches.htm)
