...
| Configuration review · Sep 14 | Captures match observed operation and intended management/routing settings | [Four captures](../configs/README.md) include tested HQ switching and management. HQ-L3/HQ-R1 now include the planned core-edge OSPF settings; both use explicit admin privilege 15 and IT-only VTY ACLs. SW1/SW2 include enable secrets for the verified elevation workflow. Non-credential switching settings remain unchanged. Credentials are redacted publicly. HQ-R1 final end marker and exact saved-file correspondence remain pending |
